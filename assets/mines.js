...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-51f856ce";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-51f856ce">'+'<span class="tile-face-51f856ce tile-front-51f856ce"></span>'+'<span class="tile-face-51f856ce tile-back-51f856ce"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-51f856ce")){return;}
var back=tile.querySelector(".tile-back-51f856ce");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-51f856ce");if(isMine){tile.classList.add("boom-51f856ce");}
if(dim){tile.classList.add("dimmed-51f856ce");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-51f856ce")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-51f856ce");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-51f856ce")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-51f856ce");void panelEl.offsetWidth;panelEl.classList.add("jolt-51f856ce");}
endRound();}
